...
export default function CampsCreate() {
  ...
  const onSubmit = async (data: CampFormData) => {
    try {
      ...
      setError(err.response?.data?.message || 'Failed to create medical camp');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="p-4 sm:p-6 max-w-2xl">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold mb-6">Create Medical Camp</h1>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 sm:space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <div>
              ...
              />
              {errors.campDate && (
                <span className="text-red-500 text-sm">{errors.campDate.message}</span>
              )}
            </div>
