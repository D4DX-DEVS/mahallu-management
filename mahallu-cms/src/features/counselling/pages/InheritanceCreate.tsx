...
export default function InheritanceCreate() {
  ...
  const { fields: heirsFields, append: appendHeir, remove: removeHeir } = useFieldArray({
    control,
    name: 'heirs',
  });
  ...
  return (
    <div className="flex-1 overflow-auto">
      <div className="p-4 sm:p-6 max-w-2xl">
        <h1 className="text-2xl sm:text-3xl font-bold mb-6">Create Inheritance Case</h1>
                ...
                {heirsFields.map((field, idx) => (
                  <Card key={field.id} className="p-4 bg-gray-50">
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-medium mb-1">Heir Name *</label>
                        <input
                          {...register(`heirs.${idx}.name`, { required: 'Name is required' })}
                          type="text"
                          placeholder="Full name"
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                        />
                        {errors.heirs?.[idx]?.name && (
                          <p className="text-red-600 text-xs mt-1">{errors.heirs[idx]?.name?.message}</p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            {...register(`heirs.${idx}.relation`, { required: 'Relation is required' })}
                            type="text"
                            placeholder="e.g., Son, Daughter, Wife"
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"
                          />
                          {errors.heirs?.[idx]?.relation && (
                            <p className="text-red-600 text-xs mt-1">{errors.heirs[idx]?.relation?.message}</p>
                          )}
                        </div>
                        <div>
