...
export default function EditMahalluCategory() {
  ...
  useEffect(() => {
    ...
    if (stateCategory) {
      ...
      masterAccountService
        .getAllCategories({ limit: 1000, scope: 'mahallu' })
        .then((r) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(errorMessage(err, { action: 'update category. please try again' }));
    } finally {
      setSaving(false);
    }
  };
        ...
        <Card className="space-y-4">
          ...
          <div className="flex gap-2 flex-col-reverse sm:flex-row sm:justify-end sm:gap-3 sm:items-center pt-2">
            <Button
              type="button"
              variant="secondary"
              onClick={() => navigate(ROUTES.MAHALLU_FINANCE.CATEGORIES)}
            >
