...
export default function DevelopmentIndex() {
  ...
  useEffect(() => {
    ...
    reportService
      ...
      .catch((error) => {
        console.error('Failed to load development index:', error);
  ...
  if (!data) return <div className="p-4">Failed to load development index</div>;
  ...
  return (
    <div className="p-4">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
        <h1 className="text-2xl font-bold">Development Index</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
        {data.dimensions.map((d) => (
          <Card key={d.key} className="p-3 sm:p-4">
            ...
            <div className="mt-2 text-[11px] sm:text-xs text-gray-500 break-words">
              {weakestIndicator(d.indicators)}
            </div>
            {data.weakest.includes(d.key) && (
              <div className="mt-1 text-[11px] sm:text-xs font-medium text-red-600">Needs attention</div>
            )}
          </Card>
        ))}
      </div>
