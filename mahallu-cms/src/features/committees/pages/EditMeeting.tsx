...
const meetingSchema = z.object({
  ...
  minutes: z.string().max(3000, 'Please keep the minutes to 3000 characters or less.').optional(),
...
]; /** Attendance comes back either populated or as raw ids, depending on the endpoint. */
const attendanceIds = (attendance: Member[] | string[] | undefined): string[] =>
  (attendance || [])
    .map((entry: any) => (typeof entry === 'string' ? entry : entry?.id))
    .filter(
      Boolean
    ); /** <input aria-label="Value" type="datetime-local"> wants 'YYYY-MM-DDTHH:mm' in local time, not an ISO string. */
...
export default function EditMeeting() {
  ...
  const {
    ...
    defaultValues: { status: 'scheduled', attendance: [] },
  });
        ...
        reset({
          committeeId:
            typeof meeting.committeeId === 'string'
              ? meeting.committeeId
              : (meeting.committeeId as any)?.id || '',
          title: meeting.title || '',
          titleMl: meeting.titleMl || '',
          meetingDate: toLocalInputValue(meeting.meetingDate),
          agenda: meeting.agenda || '',
          agendaMl: meeting.agendaMl || '',
          minutes: meeting.minutes || '',
          status: meeting.status || 'scheduled',
          attendance: attendanceIds(meeting.attendance),
        });
      } catch (err: any) {
  ...
  const toggleAttendance = (memberId: string) => {
    if (selectedAttendance.includes(memberId)) {
      setValue(
        'attendance',
        selectedAttendance.filter((entry) => entry !== memberId)
      );
    } else {
