...
export default function VolunteerDetail() {
  ...
  const toggleWing = (wing: string) => {
    ...
    setFormData((prev: any) => ({
      ...prev,
      wings: prev.wings.includes(wing) ? prev.wings.filter((w: string) => w !== wing) : [...prev.wings, wing],
    }));
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(errorMessage(err, { action: 'update volunteer' }));
    } finally {
      setSaving(false);
    }
  };
  ...
  if (!volunteer) {
    return (
      <Card>
        <div className="py-8 text-center text-red-600">Volunteer not found</div>
      </Card>
    );
  }
          ...
          {editMode && formData ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-label text-gray-500 uppercase">Contact</label>
                  <p className="text-sm font-medium text-gray-900">{volunteerContact}</p>
                </div>
                <div>
                  <label className="text-label text-gray-500 uppercase">Status</label>
                  <p
              ...
              <div>
                <label className="text-label text-gray-500 uppercase block mb-2">Wings</label>
              ...
              <div>
                <label className="text-label text-gray-500 uppercase block mb-2">Service Types</label>
                <div className="flex flex-wrap gap-2">
              ...
              <div>
                <label className="text-label text-gray-500 uppercase block mb-2">Availability</label>
              ...
              {volunteer.notes && (
                <div>
                  <label className="text-label text-gray-500 uppercase block mb-2">Notes</label>
