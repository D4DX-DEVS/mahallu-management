...
export default function DeathRegistrationsList() {
  ...
  const handleExport = async (type: 'csv' | 'json' | 'pdf') => {
    try {
      ...
      toast.error(error?.message || "Couldn't export death registrations");
    } finally {
  ...
  const columns: TableColumn<DeathRegistration>[] = [
    { key: 'deceasedName', label: 'Deceased Name', width: '11.25rem', sortable: true },
    {
      key: 'deathDate',
      label: 'Death Date',
      width: '9.5rem',
    ...
    { key: 'placeOfDeath', label: 'Place', width: '6.5rem' },
    {
      key: 'status',
      label: 'Status',
      width: '7.25rem',
