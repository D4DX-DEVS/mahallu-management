...
export default function NikahRegistrationsList() {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      setError(err.response?.data?.message || 'Failed to fetch nikah registrations');
  ...
  const handleExport = async (type: 'csv' | 'json' | 'pdf') => {
    try {
      ...
      toast.error(error?.message || 'Failed to export nikah registrations');
    } finally {
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">Nikah Registrations</h1>
            <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Manage nikah registrations</p>
          </div>
          <Breadcrumb items={[{ label: 'Dashboard', path: '/dashboard' }, { label: 'Nikah Registrations' }]} />
        </div>
